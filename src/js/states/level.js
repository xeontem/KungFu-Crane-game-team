...
export default class extends WithControlls {
  ...
  create() {
    ...
    this.mainPlayerHP = this.add.text(
        ...
        { font: `${gameState.gameHeight / 32.8}px Orbitron`, fill: '#dddddd' });
    this.mainPlayerHP.setOrigin(0.5);
  ...
  update() {
    ...
    if (this.time.now < this.countdown + 2000) {
      ...
      if (this[KEYS.UP.UP_ONCE]) {
        // this.mainPlayer.anims.play('move_up_end', true);
      ...
      if (this[KEYS.UP.DOWN_ONCE]) {
        this.mainPlayer.anims.play('move_up_start');
      }
      if (this[KEYS.UP.UP_ONCE]) {
        this.mainPlayer.anims.play('move_up_end');
      }

      if (this[KEYS.DOWN.DOWN_ONCE]) {
        this.mainPlayer.anims.play('move_down_start');
      }
      if (this[KEYS.DOWN.UP_ONCE]) {
        this.mainPlayer.anims.play('move_down_end');
      }
      ...
      if (this[KEYS.LEFT.DOWN]) {
        this.mainPlayerContainer.x += -gameState.mainPlayerSpeed;
        // this.exhaust1.setScale(0.1);
        // this.exhaust2.setScale(0.1);
        // this.exhaust1.y = -17;
        // this.exhaust2.y = 14;
      }
      ...
      if (this[KEYS.UP.DOWN]) {
        this.mainPlayerContainer.y += -gameState.mainPlayerSpeed;
        // if (gameState.mainPlayerHP <= 1) {
    ...
    //   if (this[KEYS.CHANGE_WEAPON.DOWN_ONCE]) {
    ...
    //   // once handlers
    //   if (this[KEYS.MENU.DOWN_ONCE]) {
    //     invokePauseMenu.apply(this);
    //   }

    //   if (this[KEYS.SAVE.DOWN_ONCE]) {
    //     saveGame();
    //   }

    //   if (this[KEYS.LOAD.DOWN_ONCE]) {
